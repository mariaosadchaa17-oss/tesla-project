...
// ── Ця функція викликається з index.html після аутентифікації ──
function startAppWithUid(uid) {
  // Кожен користувач має свої колекції в Firestore під своїм uid
  ...
  form.addEventListener('submit', async e => {
    e.preventDefault();
    const amount=Number(amountInput.value)||0, tip=Number(tipInput.value)||0;
    if (amount<=0 && tip<=0) return;
    const data = { amount, tip, total:amount+tip, paymentMethod:getPayment(),
      createdAt:firebase.firestore.FieldValue.serverTimestamp(), dateKey:dateKeyOf(new Date()) };
  ...
  document.querySelectorAll('.quick-tip-btn').forEach(btn => btn.addEventListener('click', ()=>{ tipInput.value=btn.dataset.val; }));
  ...
  expenseForm.addEventListener('submit', async e => {
    ...
    if (amount<=0) return;
    await expensesRef.add({ category:expenseCategory.value, amount, dateKey:todayKey(),
  ...
  saveKmBtn.addEventListener('click', async () => {
    ...
    await tripsRef.add({ amount:0,tip:0,total:0,km,kmOnly:true,paymentMethod:'cash',
      createdAt:firebase.firestore.FieldValue.serverTimestamp(), dateKey:dateKeyOf(new Date()) });
  ...
  // ── Render ──
  const sumRange=(from,to)=>allTrips.filter(t=>!t.kmOnly&&t.dateKey>=from&&t.dateKey<=to).reduce((s,t)=>s+t.total,0);
  ...
  function render() {
    ...
    const ttotal  = todayT.reduce((s,t)=>s+t.total,0);
    const tfare   = todayT.reduce((s,t)=>s+t.amount,0);
    const texTotal= todayEx.reduce((s,x)=>s+x.amount,0);
    const tcomm   = tfare*commissionPercent/100;

    todayTotalEl.textContent      = ttotal.toFixed(0);
    todayTipsEl.textContent       = todayT.reduce((s,t)=>s+t.tip,0).toFixed(0);
    todayCountEl.textContent      = todayT.length;
    weekTotalEl.textContent       = weekT.reduce((s,t)=>s+t.total,0).toFixed(0);
    monthTotalEl.textContent      = monthT.reduce((s,t)=>s+t.total,0).toFixed(0);
    todayExpensesEl.textContent   = texTotal.toFixed(0)+' грн';
    todayCommissionEl.textContent = '= '+tcomm.toFixed(0)+' грн';
    todayNetEl.textContent        = (ttotal-tcomm-texTotal).toFixed(0)+' грн';
    ...
    todayT.forEach(t=>{
      ...
      li.innerHTML=`<span class="history-time">${time}</span><span class="history-amounts">${pi} ${t.amount} грн <span class="history-tip">+${t.tip} чай</span></span><span class="history-total">${t.total} грн</span><button class="edit-btn">✎</button><button class="delete-btn">×</button>`;
  ...
  function renderHeatmap() {
    ...
    allTrips.forEach(t=>{
      ...
      const d=t.createdAt.toDate(), dow=d.getDay()===0?6:d.getDay()-1, b=getTimeBucket(d.getHours());
      if (b===null) return; grid[dow][b]+=t.total;
    });
    ...
    grid.forEach((row,i)=>{
      const d=wd[i], ds=dateKeyOf(d), isT=ds===ts;
  ...
  function renderRange() {
    ...
    const f=allTrips.filter(t=>{ if(t.kmOnly)return false; if(from&&t.dateKey<from)return false; if(to&&t.dateKey>to)return false; return true; });
    rangeTotalEl.textContent=f.reduce((s,t)=>s+t.total,0).toFixed(0)+' грн';
  ...
  exportPdfBtn.addEventListener('click', async () => {
    const old=exportPdfBtn.textContent; exportPdfBtn.textContent='Генерація...'; exportPdfBtn.disabled=true;
    const from=rangeFrom.value, to=rangeTo.value, isRange=from||to;
    let rt,re,title,period,fname;
    if (isRange) {
      rt=allTrips.filter(t=>{ if(t.kmOnly)return false; if(from&&t.dateKey<from)return false; if(to&&t.dateKey>to)return false; return true; });
      re=allExpenses.filter(x=>{ if(from&&x.dateKey<from)return false; if(to&&x.dateKey>to)return false; return true; });
    ...
    const total=rt.reduce((s,t)=>s+t.total,0), fare=rt.reduce((s,t)=>s+t.amount,0);
    const tips=rt.reduce((s,t)=>s+t.tip,0), count=rt.length;
    const expenses=re.reduce((s,x)=>s+x.amount,0), comm=fare*commissionPercent/100, net=total-comm-expenses;
    try {
      ...
      doc.addFileToVFS('Roboto-Regular.ttf',fb64); doc.addFont('Roboto-Regular.ttf','Roboto','normal'); doc.setFont('Roboto','normal');
      doc.setFontSize(18); doc.text(title,14,20);
      doc.setFontSize(10); doc.setTextColor(120,120,120);
      doc.text('Період: '+period,14,30); doc.text('Сформовано: '+new Date().toLocaleDateString('uk-UA'),14,37);
      ...
      line('Чистими:',net.toFixed(0)+' грн'); doc.save(fname);
    } catch(err) {
      const txt=[title,'Період: '+period,'','Зароблено: '+total.toFixed(0)+' грн ('+count+' поїздок)','Чайові: '+tips.toFixed(0)+' грн','Витрати: '+expenses.toFixed(0)+' грн','Комісія: '+comm.toFixed(0)+' грн','Чистими: '+net.toFixed(0)+' грн'].join('\n');
      const a=document.createElement('a'); a.href=URL.createObjectURL(new Blob(['\uFEFF'+txt],{type:'text/plain;charset=utf-8;'})); a.download=fname.replace('.pdf','.txt'); a.click();
    } finally { exportPdfBtn.textContent=old; exportPdfBtn.disabled=false; }
  });
}
